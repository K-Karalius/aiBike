...
import { CreateStationRequest, GetStationsRequest, Station } from "@/interfaces/station";
...
export const getStations = async (data: GetStationsRequest): Promise<Station[]> => {
    const response = await api.get("/station", {params: data});
    return response.data;
}

export const deleteStation = async (data: CreateStationRequest): Promise<AxiosResponse> => {
